...
class Bot(BaseBot):
    ...
    async def on_start(self, session_metadata):
        ...
        if self.bot_pos:
            await self.highrise.walk_to(Position(5.0 , 1.5 , 18.0 , "FrontRight"))
    ...
    async def on_chat(self, user: User, message: str) -> None:
            ...
            if message.startswith("!wallet") and user.username == "____Rayen____" or user.username == "ShoKytoo" or user.username == "ShoKytoo":
                  wallet = (await self.highrise.get_wallet()).content
                  await self.highrise.chat(f"The bot wallet contains {wallet[0].amount} {wallet[0].type}")
                
            if message.lower().startswith("!tipme ") and user.username=="____Rayen____":
                try:
            ...
            if message.lower().startswith("!tipall ") and user.username == "____Rayen____":
              parts = message.split(" ")
              if len(parts) != 2:
              ...
              for room_user, pos in room_users.content:
                ...
                if self.pos1 and self.pos2:
                    ...
                    await self.highrise.chat("✅️Dancefloor Started.")          
                    await self.highrise.chat("❌️Want to stop the dancefloor? use: !stop")
                    self.pos1 = None
                    self.pos2 = None
                else:
                ...
                self.on_dance_floor = []
                await self.highrise.chat("❌️Dancefloor Stopped!")
                self.save_loc_data()
