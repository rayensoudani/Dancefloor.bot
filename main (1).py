...
class Bot(BaseBot):
    ...
    async def on_start(self, session_metadata):
        ...
        if self.bot_pos:
            await self.highrise.walk_to(Position(5.5 , 1.5 , 17.5 ,"FrontRight"))
    ...
    async def on_chat(self, user: User, message: str) -> None:
            ...
            if message.lower().startswith("!tipall ") and user.username == "iced_yu":
              ...
              for room_user, pos in room_users.content:
                ...
                await self.highrise.chat(f"✅️Dancefloor 1 set Configured: [{self.pos1}]")

            elif message.startswith("/dancefloor2"):

                self.pos2 = await self.get_actual_pos(user.id)
                await self.highrise.chat(f"✅️Dancefloor 2 set Configured: [{self.pos2}]")

            elif message.startswith("/check"):

                await self.highrise.chat(f"{self.on_dance_floor}")

            elif message.startswith("/start dancefloor"):

                if self.pos1 and self.pos2:
                    await self.create_dance_floor()
                    await self.highrise.chat("✅️Dancefloor created.")          
                    await self.highrise.chat("❌️Use /stop to stop Dancefloor.")
                    self.pos1 = None
                    self.pos2 = None
                else:
                    await self.highrise.chat("❌️ERROR:  Please set both /dancefloor1 and /dancefloor2 first‼️")
